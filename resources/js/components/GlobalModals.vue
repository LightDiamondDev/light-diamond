...
<template>
    <div :class="{ 'on': globalModalStore.isAuth }" class="auth-modal-window flex justify-center items-center">
        <div class="interface flex">
            ...
            <div :class="{ 'id1': authCounter == 1, 'id2': authCounter == 2, 'id3': authCounter == 3, 'id4': authCounter == 4 }" class="forms">
                ...
                <form action="" :class="{ 'on': authCounter == 1 }" class="recovery flex flex-col items-center">
                    <div class="header flex justify-between items-center">
                        ...
                        <button class="flex justify-center items-center m-4" @click="globalModalStore.isAuth = !globalModalStore.isAuth" type="button">
                    ...
                    <span class="task text-[0.8rem] mb-2 mt-2">
                        ...
                        <span :class="{ 'error': !successNickname, 'success': successNickname}" class="status text-[0.8rem] mt-1">
                            Ошибка тект с ошибкой текст!
                    </span>
                        <span class="subtitle text-[1.1rem] m-2">Повторный пароль</span>
                        <label for="recovery-repeat-password">
                            <input class="text-[0.9rem]" id="recovery-repeat-password" placeholder="Повторный пароль" type="password">
                        </label>
                        <span :class="{ 'error': !successNickname, 'success': successNickname}" class="status text-[0.8rem] mt-1">
                            Ошибка тект с ошибкой текст!
                    </span>
                    ...
                    <button class="auth-button-container button-container flex justify-center items-center" type="submit">
                ...
                <form action="" :class="{ 'on': authCounter == 3 }" class="login flex flex-col items-center">
                    ...
                    <span class="subtitle text-[1.1rem] m-2">
                        ...
                        <span :class="{ 'error': !successNickname, 'success': successNickname}" class="status text-[0.8rem] m-2">
                            Ошибка тект с ошибкой текст!
                    </span>
                ...
                <form action="" :class="{ 'on': authCounter == 4 }" class="register flex flex-col items-center">
                    ...
                    <fieldset class="flex flex-col items-center">
                        ...
                        <label for="register-email">
                            <input class="text-[0.9rem]" id="register-email" placeholder="steve@minecraft.net" type="email">
                        </label>
                        <span :class="{ 'error': !successNickname, 'success': successNickname}" class="status text-[0.8rem] m-2">
                            Ошибка тект с ошибкой текст!
                    </span>
                        ...
                        <label for="register-repeat-password">
                            <input class="text-[0.9rem]" id="register-repeat-password" placeholder="Повторный пароль" type="password">
                        </label>
                        <span :class="{ 'error': !successNickname, 'success': successNickname}" class="status text-[0.8rem] m-2">
                            Ошибка тект с ошибкой текст!
                    </span>
...
.auth-modal-window .forms.id1 form { transform: translateY(0); }
.auth-modal-window .forms.id2 form { transform: translateY(-100%); }
.auth-modal-window .forms.id3 form { transform: translateY(-200%); }
.auth-modal-window .forms.id4 form { transform: translateY(-300%); }
.auth-modal-window form {
    transition: 1s;
    opacity: 0;
}
...
.auth-modal-window form .auth-button-container { width: 85%; }
...
.auth-modal-window form fieldset * { color: var(--primary-text-color); }
...
.auth-modal-window form fieldset input::placeholder { font-size: .8rem; }
.auth-modal-window form fieldset .status.error { color: #ff3050; }
.auth-modal-window form fieldset .status.success { color: var(--hover-text-color); }
.auth-modal-window form .auth-options { width: 100%; }
.auth-modal-window form .auth-options .auth-remember-line { width: fit-content; }
.auth-modal-window form .auth-options .auth-remember-line input { width: fit-content; }
...
#auth-remember-checkbox:focus-visible + .ld-checkbox + .auth-remember-me { color: var(--hover-text-color); }
#auth-remember-checkbox:checked + .ld-checkbox .icon-tick { opacity: 1; }
...
.auth-modal-window form .mob.iron-golem { height: 216px; }
...
.auth-modal-window form .mob .animation-dancing-multicoloured-parrot { animation: parrot-flip-animation 10s infinite; }
...
.auth-modal-window .illustration .background-auth { animation: background-auth-animation 20s infinite; }
...
.back-background { animation: background-back-animation 60s infinite; }
...
{
    .auth-modal-window .illustration { display: none; }
}
...
{
    .auth-modal-window form h1 { font-size: 1.2rem; }
    .auth-button-container .action-button .text { font-size: 0.8rem; }
    .auth-modal-window form fieldset .status {
        font-size: 0.7rem;
        line-height: 1.1;
    }
