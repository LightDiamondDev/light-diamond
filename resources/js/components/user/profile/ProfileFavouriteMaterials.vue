...
import ProfileMaterialCard from '@/components/user/profile/ProfileMaterialCard.vue'
...
function loadMaterials() {
    ...
    axios.get(`/api/users/${props.userId}/favourite-materials`, {params: loadRequestData}).then((response) => {
        ...
        <div v-else>
            ...
            >
                <p v-if="authStore.id === userId" class="text-muted text-center text-[14px] max-w-[480px]">
                    <span>У Вас пока ещё нет любимых Материалов, ещё найдёте! ;D</span>
                </p>
                <p v-else class="text-muted text-center text-[14px] max-w-[480px]">
                    <span>У </span>
                    <span>{{ username }}</span>
                    <span> пока ещё нет любимых Материалов.</span>
                </p>
            ...
            <div v-else class="flex flex-col" :class="{'paginator-refresh': isLoading}">
                <ProfileMaterialCard v-for="material in materials" :material="material"/>
            </div>
        </div>
        <Paginator
