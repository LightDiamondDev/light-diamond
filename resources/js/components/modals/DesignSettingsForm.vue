<script setup lang="ts">
import Button from '@/components/elements/Button.vue'
import Switcher from '@/components/elements/Switcher.vue'
import useDesignManager from '@/design-manager'
const designManager = useDesignManager()
</script>

<template>
    <div class="flex flex-col items-center p-1">
        <button class="settings-button flex justify-between items-center m-1 p-2"
                @click="designManager.switchLightTheme()"
        >
            <span v-if="designManager.isLightThemeVisible()" class="settings-icon icon icon-sun"></span>
            <span v-else class="settings-icon icon icon-moon"></span>
            <span class="text-base">Тёмная Тема</span>
            <Switcher :active=designManager.isLightThemeVisible() />
        </button>

        <button class="settings-button flex justify-between items-center m-1 p-2"
            @click="designManager.switchHeaderFixed()"
        >
            <span v-if="designManager.isHeaderFixedVisible()" class="settings-icon icon icon-fixed-header"></span>
            <span v-else class="settings-icon icon icon-free-header"></span>
            <span class="text-base">Свободная Шапка</span>
            <Switcher :active=designManager.isHeaderFixedVisible() />
        </button>

        <button class="settings-button xl:flex justify-between items-center hidden m-1 p-2" @click="designManager.switchDesktopSidebar()">
            <span v-if="designManager.isDesktopSidebarVisible()" class="icon icon-units"></span>
            <span v-else class="settings-icon icon icon-cross"></span>
            <span class="text-base">Боковое Меню</span>
            <Switcher :active=!designManager.isDesktopSidebarVisible() />
        </button>
    </div>
</template>

<style>
.design-settings-form .settings-button {
    height: 72px;
    width: 98%;
}
.design-settings-form .settings-button .icon {
    height: 32px;
    width: 32px;
}
</style>
