...
const isLoading = ref(!!props.slug)
...
function loadPost() {
    ...
    axios.get(`/api/posts/${props.slug}`).then((response) => {
        ...
        if (Object.keys(post).length !== 0 && post.version!.author_id === authStore.id) {
            postVersion.value = {...post.version!}
            delete postVersion.value.id
            postVersion.value.post_id = post.id
            postVersion.value.post = post
        }
